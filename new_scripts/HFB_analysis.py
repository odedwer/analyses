...
power_short_H = tfr_multitaper(epochs[stimuli[::2]], freqs=freqsH, average=False,
                               n_cycles=freqsH / 10, use_fft=True,
                               return_itc=False, decim=3, n_jobs=12)
power_short_H = power_short_H.average()

power_long_L = tfr_multitaper(epochs[stimuli[1::2]], freqs=freqsL, average=False,
                              n_cycles=freqsL / 5, use_fft=True,
                              return_itc=False, decim=3, n_jobs=12)
power_long_L = power_long_L.average()

power_short_L = tfr_multitaper(epochs[stimuli[::2]], freqs=freqsL, average=False,
                               n_cycles=freqsL / 5, use_fft=True,
                               return_itc=False, decim=3, n_jobs=12)
...
# %%
dt_scores = [
    duration_tracking_new(epochs_HFB_L, epochs_HFB_S, ch, time_diff=[time_end_short + .1, time_end_long + .1], nperm=100)[0]
    for ch in epochs_hilb.ch_names[:-9]];
print("Done with duration tracking")
onset_resp_score = [total_onset_power(epochs_hilb, ch, nperm=100)[0] for ch in epochs_hilb.ch_names[:-9]]
# %%
...
onset_resp_df = pd.DataFrame(
    {"electrode": raw_unfiltered.ch_names[0:len(onset_resp_score)], "onset_resp": onset_resp_score})
