...
def read_bdf_files(preload=True):
    ...
    filename = join(BASE_DATA_DIR, subject_string, "eeg", "raw",
                    subject_string + "_task-" + modality + "-raw.bdf")
...
def read_fif_files(preload=True):
    ...
    subject_string = f"sub-{subject_num}"
    filename = join(BASE_DATA_DIR, subject_string, "eeg", "raw",
                    subject_string + "_task-" + modality + "-raw.fif")
    return mne.io.read_raw_bdf(filename, preload=preload)
...
raw_filtered = mne.io.read_raw_fif(join(save_dir, filtered_filename),
                                   preload=True)
raw_unfiltered = mne.io.read_raw_fif(join(save_dir, unfiltered_filename),
                                     preload=True)
...
# %% variance ratio
events = mne.find_events(raw_filtered, stim_channel="Status", mask=255,
                         min_duration=2 / raw_filtered.info['sfreq'])
...
raw_filt = raw_unfiltered.copy().filter(l_freq=1,
                                        h_freq=30)  # performing filtering on copy of raw data, not on raw itself or epochs
raw_filt.notch_filter([50, 100, 150])  # notch filter
events = mne.find_events(raw_unfiltered, stim_channel="Status", mask=255,
                         min_duration=2 / raw_unfiltered.info['sfreq'])
...
print(
    f"removed {n_trials - len(epochs_filt_clean)} trials by peak to peak rejection with threshold {np.round(threshold['eeg'], 5)} V on eeg and {np.round(threshold['eog'], 5)} V on eog")
epochs_filt_clean.plot()

# %% save
exclusions = pd.DataFrame({"excluded": ica.exclude})
exclusions.to_csv(join(save_dir,
                       f"sub-{subject_num}_task-{modality}-{low_cutoff_freq:.2f}hpf-overweighted-ica-rejected.csv"))
epochs_filt_clean.save(
    join(save_dir, f"sub-{subject_num}_task-{modality}-1-30-bp-epo.fif"))
raw_unfiltered.save(join(save_dir,
                         f"sub-{subject_num}_task-{modality}-unfiltered-clean-raw.fif"))
